...
public class Row extends HBox {
    ...
    private List<SplitFlap>               splitFlaps;


    public Row(final CharacterSet characterSet, final int _noOfCharacters) {
    ...
    public Row(final CharacterSet characterSet, final int _noOfCharacters, final double flipTime, final boolean axisVisible, final boolean shaded) {
        this(characterSet, SplitFlapFont.BEBAS, _noOfCharacters, flipTime, Constants.GRAY, Constants.WHITE, axisVisible, shaded);
    }
    public Row(final CharacterSet characterSet, final SplitFlapFont splitFlapFont, final int noOfCharacters, final double flipTime) {
    ...
    public Row(final CharacterSet characterSet, final SplitFlapFont splitFlapFont, final int noOfCharacters, final double flipTime, final Color backgroundColor, final Color textColor, final boolean axisVisible, final boolean shaded) {
        ...
        this._shaded          = shaded;
        this.splitFlaps       = new ArrayList<>(noOfCharacters);
    ...
    private void initGraphics() {
        for (int i = 0 ; i < this.noOfCharacters ; i++) { this.splitFlaps.add(new SplitFlap(getCharacterSet(), getSplitFlapFont(), getBackgroundColor(), getTextColor(), this.flipTime, isAxisVisible(), isShaded())); }
        getChildren().setAll(this.splitFlaps);
    }
    ...
    public ObjectProperty<CharacterSet> characterSetProperty() {
        if (null == this.characterSet) {
            this.characterSet = new ObjectPropertyBase<>(_characterSet) {
                ...
                @Override public Object getBean() { return Row.this; }
    ...
    public void setText(final String text) {
        if (text.length() > this.noOfCharacters) { throw new IllegalArgumentException("Text too long (max " + this.splitFlaps.size() + " characters)"); }
        for (int i = 0 ; i < text.length(); i++) { this.splitFlaps.get(i).setCharacter(String.valueOf(text.charAt(i))); }
    }

    public void reset() {
        for (int i = 0 ; i < this.noOfCharacters; i++) { splitFlaps.get(i).reset(); }
    }

    public void selfCheck() {
        for (int i = 0 ; i < noOfCharacters; i++) { splitFlaps.get(i).selfCheck(); }
    }
